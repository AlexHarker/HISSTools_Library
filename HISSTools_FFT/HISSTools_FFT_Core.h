...
template <class T, class U, class V, class W> void fft_passes(Split<W> *input, Setup<W> *setup, uintptr_t fft_log2)
{
...
void fft_passes_simd(Split<float> *input, Setup<float> *setup, uintptr_t fft_log2)
{
    if (SIMD_Support == kAVX256)
        fft_passes<SSEFloat, SSEFloat, AVX256Float>(input, setup, fft_log2);
    else
        fft_passes<SSEFloat, SSEFloat, SSEFloat>(input, setup, fft_log2);
}
...
template <class T>void hisstools_fft(Split<T> *input, Setup<T> *setup, uintptr_t fft_log2)
{
    if (fft_log2 >= 4)
    {
        if (reinterpret_cast<uintptr_t>(input->realp) % 16 || reinterpret_cast<uintptr_t>(input->imagp) % 16 || SIMD_Support == kNone)
            fft_passes<Scalar<T>, Scalar<T>, Scalar<T> >(input, setup, fft_log2);
        else
